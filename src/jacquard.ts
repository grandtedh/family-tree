...
export default async function jacquard(familyTree: [number, number][]): Promise<[number, number, number, number, number, number, number, number, number]> {
    const module = await createModule()
    const ptr = module._malloc(familyTree.length * 2 * SIZEOF_SHORT)
    module.HEAPU16.set(familyTree.flat(1), ptr / SIZEOF_SHORT)

    const outPtr = module._jacquard(ptr, familyTree.length)
    module._free(ptr)
